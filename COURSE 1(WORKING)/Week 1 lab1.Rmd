...
###excercise plot the boys and girls 
```{r}
#ggplot(data=arbuthnot,aes())
```
...
Y=(arbuthnot$boys/arbuthnot$total)*100
ggplot(data = arbuthnot,aes(x=year,y=(arbuthnot$boys/arbuthnot$total) * 100)) + geom_line() 
```
...
```{r}
h<-(mean(arbuthnot$boys/arbuthnot$total))*100
h
#on avg the birth of boys are 1.7% more than birth of girls.
```
...
ggplot(data = present,aes(x=present$year,y=present$prop_boy_girl)) + geom_line()
```
...
```{r most-total-births}
# type your code for Question 8 her
