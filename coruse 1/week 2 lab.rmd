...
ggplot(data = nycflights,aes(x=dep_delay))+geom_histogram(binwidth =70)
...
```{r rdu-flights-hist}
# the rdu_flights is a new data set that has only the flights with destinations RDU.
...
```
```{r}
#rdu_flights %>% summarise(summary(dep_delay),sd(dep_delay))
```
...
statistics.
```{r}
ggplot(data = nycflights,aes(x=origin)) + geom_bar()
```
...
sfo_feb_flights %>% 
  group_by(carrier) %>% 
  summarise(iqr=IQR(arr_delay),med=median(arr_delay))
```
...
nycflights %>% group_by(month) %>% summarise(mean_delay=mean(dep_delay))
```
...
nycflights %>% group_by(month) %>% summarise(median_delay=median(dep_delay))
```
...
nycflights %>% group_by(origin) %>% 
  summarise(ot_flights=sum(dep_type=="on time"),dl_flights=sum(dep_type=="delayed"),
            total_fights=ot_flights+dl_flights)
```
...
nycflights <- nycflights %>% 
  ...
  #select(avg_speed_mph,tailnum) %>% 
  arrange(desc(avg_speed_mph))
```
```{R}
c <- nycflights %>% select(avg_speed_mph,tailnum)
head(c)
```
...
ggplot(data = nycflights,aes(x=distance , y=avg_speed_mph )) + geom_point()
```
...
nycflights <- nycflights %>% mutate(arr_type =ifelse(arr_delay >0,"delayed","on time"))
  #group_by(dep_type) %>% 
  #summarise(propotion=sum(dep_type=="on time" & arr_type == "on time")/n())
nycflights <- nycflights %>% 
  mutate(dept_and_arr=ifelse(dep_type=="on time" & arr_type=="on time","both on time","delayed"))

g=nycflights %>% select(arr_type,dep_type,dept_and_arr)
head(g)
```
```{r}
nycflights %>% group_by(dep_type) %>% summarise(proportion=sum(arr_type== "on time")/n())
```
